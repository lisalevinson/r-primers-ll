...
Let's apply `pivot_wider()` to a real world inquiry. The plot below visualizes an aspect of the `babynames` data set from the **babynames** package. (See [Work with Data](/transform-data/02-tibbles/) for an introduction to the `babynames` dataset.)

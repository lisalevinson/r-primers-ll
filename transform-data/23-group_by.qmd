...
For example, the result below is grouped into rows that have the same combination of `year` and `sex` values: babies marked in "M" in 1880 are treated as one group, babies marked "F" in 1880 as another group and so on.
...
In the code chunk below, use `group_by()`, `summarize()`, and `arrange()` to display the ten most popular names. Compute popularity as the total number of children of a single sex given a name. In other words, the total number of babies marked "M" named "Kelly" should be computed separately from the total number of babies marked "F" named "Kelly".
